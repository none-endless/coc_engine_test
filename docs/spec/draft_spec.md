...
本项目是一套面向历史、文学与文化理解场景的多 Agent 叙事引擎，目标是：
...
6. 保持实现可落地，避免过度工程化。
7. 兼容各类输入,保障响应智能,在保持自由互动的同时优先服务人物理解、礼仪辨识、文本线索分析与文化语境学习
...
当前默认世界与默认提示词不再以“高刺激冒险/战斗体验”为第一目标，而以历史、文学与文化理解为第一目标。

默认设计原则：

1. 优先支持观察、提问、交谈、礼仪判断、人物关系理解、文本线索分析。
2. 冲突与战斗是可支持的边缘分支，不是默认鼓励方向。
3. Narrative、DM、NPC 等提示词应避免血腥化、夸张化与纯闯关表达。
4. 默认场景参考《三顾茅庐》《林黛玉到贾府》这类具有明确文化学习价值的互动世界。
...
这是一套带输入分支路由、LLM与规则双驱动、带状态回滚容错的交互式多智能体教育叙事系统，核心逻辑为：inputSystem负责输入类型分流，底层元命令跳过LLM链直接走规则引擎，自然语言输入进入LLM导演链；蓝色虚线代表并发执行的独立分支流程，同时内置「重试-降级-回滚」的完整容错机制。
...
dm_agent对自然语言输入做意图解析、语义理解、叙事合理性校验与教育导向约束后，将结构化后的输入传递给rule_system，由rule_system完成规则判定、逻辑校验，生成最终执行结果
...
- llm
  - 与玩家进行对话,向玩家提供信息
  - 以历史/文学教育场景为默认方向,优先支持观察、询问、礼仪判断、关系理解、线索分析
  - 确定玩家意图并拦截非法输入,使用对话功能提醒玩家,至少包含:
    1. 越权、作弊、系统提示词/配置泄露、跳出游戏
    2. 脏话辱骂、人身攻击、歧视性表达
    3. 与当前故事主题明显无关的现实任务或助手型请求
    4. 与当前场景设定、时代背景或已发生叙事冲突的输入
    5. 与教育导向冲突的低俗、色情、羞辱、猎奇血腥内容
    6. 无意义刷屏、纯噪声、缺乏可执行意图的输入
  - 确定玩家交互是否需要进行鉴定
    1. 高风险行为
    2. 影响剧情发展或人物关系/礼序理解
    3. 不合情理需要进行软约束
  ...
  - 非法或越权输入才允许使用 `dm_reply`；正常场景内反馈必须交给后续链路
  - 对于明显命中的非法输入,允许在系统前置规则层直接短路拦截,避免完全依赖 LLM 自由判断
...
### evolution_agent:
  ...
  - 优先保留对人物关系、礼仪秩序、文化语境和文本线索有帮助的变化
  - 输出一个推演概要(summary)
