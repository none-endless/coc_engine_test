...
- llm
  ...
  - 如果不需要鉴定就不输出鉴定相关的信息并输出给evolution_agent,需要鉴定就输出给ruleSystem
...
### 一致性维护_agent:
- llm
  - 读取世界信息、叙事信息和最近变更日志，检查世界状态与叙事状态是否存在冲突
  - 汇总冲突点并输出一致性修正建议，供系统执行修补与回写
  - 根据系统反馈调整输出，直到修正建议能够被系统接受
- 配套系统
  - 按固定回合间隔触发一致性检查与修复流程
  - 校验世界状态与叙事状态是否自洽，包括 location、属性状态、关系、叙事因果链等
