...
1.  **DMAgent 实现**：
    ...
    -   **DM 直出分流**：当 `routing_hint=null` 且 `dm_reply` 非空时，本回合直接返回 DM 回复，不再进入 Evolution。
2.  **EvolutionAgent 实现**：
    -   接收鉴定结果，或接收 `routing_hint=null 且 dm_reply 为空` 的自然语言意图。
    -   生成 **推演概要 (ShortSummary)**，判断变化是否对玩家可见。
    -   **不可见变更**：直接写入 `回合因果链 (e7)`，跳过叙事生成。

#### 验收需求
-   DMAgent 输出格式包含严格的 `鉴定对象` 字段且 ID 真实存在。
-   `routing_hint=null` 且 `dm_reply` 非空时，不再触发 Evolution 链路。
